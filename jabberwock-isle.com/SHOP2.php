...
function getStudents($itemid, $affinity) {
    ...
    if (mysqli_num_rows($affinityresult) > 0) {
        ...
        while ($affinityrow = mysqli_fetch_assoc($affinityresult)) {
            echo "<img src='studentsprites/originalsprites/".$affinityrow['studentsprite']."'> <a target='_blank' href='https://jabberwock-isle.com/student.php?sid=".$affinityrow['studentid']."'>".$affinityrow['studentname']."</a> ";
        }
        echo '</td></tr>';
    }
}
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="purchaseGiftModalLabel">Purchase Item</h5>
            ...
            <p>TextArea for details</p>
            <p>Dropdown for who's purchasing</p>
            <p>Checkbox if it's a gift to someone. If unchecked, show the below:</p>
            <p>Dropdown for who's receiving</p>
            <p>Textarea for gift note</p>
            <div class="modal-footer">
                <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancel</button>
                <button id="purchaseGiftConfirm" class="btn btn-success">Purchase Item</button>
            </div>
        </div>
    </div>
</div>

    ...
    if (mysqli_num_rows($reciperesult) > 0) {
        ...
        while ($reciperow = mysqli_fetch_assoc($reciperesult)) {
            ...
            if ($reciperow['type'] == 'b-craft') {
                ...
                if ($newDiff > 0) {
                    printNestedRecipe($conn, $userid, $reciperow['ingredientid'], $diff);
                }
            }
        }
        echo "</tbody></table>";
...
$("#inventorySelect").change(function() {
    if ($(this).val() == "all") {
        window.location.href = "https://jabberwock-isle.com/inventories.php";
    } else {
        window.location.href = "https://jabberwock-isle.com/inventories.php?uid=" + $(this).val();
    }
});
</script>

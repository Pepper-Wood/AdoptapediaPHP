    ...
    while ($userrow = mysqli_fetch_assoc($userresult)) { ?>
        <div class="row">
            ...
            <div class="col-lg-3 col-md-3">
                ...
                ?>
                <h4 class='inventorySectionTitle'>...</h4>
                <?php
                    $sql = "SELECT giftsinventories.studentid as studentid,giftsinventories.note as note,giftsinventories.giftid as giftid, items.itemname as itemname,items.itemimage as itemimage,students.studentname as studentname, students.studentsprite as studentsprite FROM giftsinventories LEFT JOIN items ON items.itemid=giftsinventories.itemid LEFT JOIN students ON students.studentid=giftsinventories.studentid WHERE giftsinventories.userid=".$userrow['userid']." ORDER BY giftsinventories.studentid;";
                    $result = mysqli_query($conn, $sql);
                    $currStudentID = "";
                    if (mysqli_num_rows($result) > 0) {
                        echo '<div class="card">';
...
$(".giftEditModalTrigger").click(function() {
    var currGift = $(this).attr("id");
    $("#editGiftModal_giftid").text(currGift);
    $("#editGiftModal_itemname").text($("#"+currGift+"itemname").text());
    $("#editGiftModal_note").text($("#"+currGift+"note").text());
    $("#newGiftNote").val("");
    $("#editGiftModal_giftImg").css("background-image","url("+$("#"+currGift+"img").prop("src")+")");
});
$("#editGiftConfirm").click(function() {
    $.post("editGiftNote.php", {giftid: $("#editGiftModal_giftid").text(), newgiftnote: $("#newGiftNote").val()}, function(result) {
        location.reload();
    });
});
$(".giftRemoveModalTrigger").click(function() {
    var currGift = $(this).attr("id").replace("giftRemove","gift");
    $("#removeGiftModal_giftid").text(currGift);
    $("#removeGiftModal_itemname").text($("#"+currGift+"itemname").text());
    $("#removeGiftModal_note").text($("#"+currGift+"note").text());
    $("#newGiftNote").val("");
    $("#removeGiftModal_giftImg").css("background-image","url("+$("#"+currGift+"img").prop("src")+")");
});
